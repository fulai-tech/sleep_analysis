...
| `causal` | 取 `processing_config.causal` | 见下方「因果分支覆盖范围」 |
...
管线对退化输入做三层防护，**不会静默产出缺列的表**：

| 层 | 位置 | 行为 |
|---|---|---|
| 1. NaN 守卫 | `pipeline.fill_signal_nans` | 输入信号的 NaN 用线性插值补齐并打印数量；`detect_beats` 直接收到 NaN 时抛错 |
| 2. epoch 剔除 | `pipeline.process_signal` | 一个 epoch 内 RR 间期 < 10（与 MESA 同门槛）→ **整行剔除**，不填 0 |
| 3. 输出校验 | `pipeline.validate_blocks` | 三条支路各自校验列数（ACT 370 / HRV逐epoch 30 / RRV 60），不符即抛错 |

背景：

- **为什么需要 NaN 守卫**：IIR 滤波是递归的，一个 NaN 会扩散到其后所有样本；
  非因果分支的 `filtfilt`（前向+反向）更是双向扩散。
- **为什么剔除而不是填 0**：`_hrv_median_nni = 0 ms` 在物理上不可能，填 0 会让它
  被当成测量值喂进模型。
- **为什么需要输出校验**：若某条支路的所有窗口都失败，`pd.DataFrame([{}, ...])`
  是 0 列，该支路的特征会**整块消失**（实测 RRV 60 列全没，特征表 581 → 521 列），
  而管线照常返回、不报任何错。

可调参数（`process_signal`）：`min_beats_per_epoch`（默认 10）、
`max_bad_epoch_frac`（默认 0.2，剔除比例超限即判该被试数据质量不足）。

### RR 间期归属约定（与 MESA 对齐）

`detect_beats` 输出的 `RR_Interval` 挂在**后一拍**上：

```
rr[i] = t[i] − t[i−1]        第一拍无前驱 → 填全部间期的均值
```

与 MESA 一致（`rr_utils.py`: `seconds.diff()` 后 `fillna(mean)`）。

> D04 用的是相反约定（`empkins_micro.get_rpeaks` 里 `np.ediff1d(..., to_end=0)`，
> 挂在**前一拍**、最后一拍填均值）。两种约定下每个 30 s epoch 取的间期集合错开
> 一拍，实测 HRV 逐 epoch 特征差异：`median_nni` 0.00%（中位数对平移不敏感）、
> `lf` 1.7%（受影响最大）。本管线以 MESA 为准。

### 因果分支覆盖范围（`causal=True`）

四步**全部**使用只含已见数据的统计量，因此同一段前缀信号「单独跑」与
「接上后续数据跑」结果逐位一致（流式推理可复现离线特征）：

| 步骤 | 非因果 | 因果 |
|---|---|---|
| 滤波 | `filtfilt`（零相位） | `lfilter`（正向） |
| 检测阈值 | 整段 90 分位 | 300 s 尾部滚动 90 分位 |
| 群延迟补偿 | 不补偿 | 补偿 FIR 的 `(n_taps−1)//2` |
| **异常拍剔除** | 整段 median/MAD | **300 拍尾部滚动 median/MAD** |

> ⚠️ 剔除这一步（`_remove_outliers` 的统计离群规则）曾长期使用整段统计量，
> 导致流式与离线不一致 —— 实测不可复现的峰占比：干净信号 0.06%，
> 3× 噪声 + 30 体动/h 时升到 **1.80%**。现已按 `causal` 分支。
>
> 注意生理范围规则（相邻 RR 在 [0.3, 2.0] s 内）本身只看相邻两拍，无需分支。
>
> 另外：前缀末尾约 2 s 内会有几个峰只在全量跑时出现 —— 那是**峰确认需要后一个
> 样本**的固有延迟，与因果性无关。
